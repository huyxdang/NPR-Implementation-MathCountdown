...
def reward_fn(generated_text: str, ground_truth: Dict, scale_factor: float = 1.0) -> float:
    """
    Binary verifiable reward for MATH dataset:
    ...
    is_correct = verify_answer(predicted_answer, target)
...
def _format_eval_example(example: Dict[str, Any]) -> str:
    ...
    predicted = example.get("predicted_answer", None)
    return (
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="NSR (Negative Sample Reinforcement) Experiments on MATH Dataset",
    ...
    parser.add_argument("--model_id", type=str, default="Qwen/Qwen2.5-0.5B-Instruct",
                        help="HuggingFace model ID (e.g., Qwen/Qwen2.5-0.5B-Instruct, Qwen/Qwen2.5-1.5B-Instruct, Qwen/Qwen2.5-3B-Instruct)")
...
def main() -> None:
    ...
    def build_math_dataset(data):
        """Build dataset for MATH problems."""
        examples = []
        for ex in data:
            # Format the problem using MATH template
            problem_text = ex["problem"]
            prompt = MATH_TEMPLATE.format(problem=problem_text)
            ...
            examples.append({
                "prompt": prompt,
                "answer": {"target": ex["answer"]},
            })
        return examples

    # Load MATH dataset from local files
    print("Loading MATH dataset from local files...")
    train_data = load_math_jsonl("./data/math_json/train.jsonl")
    eval_data = load_math_jsonl("./data/math_json/test.jsonl")
    
    print(f"Loaded {len(train_data)} training examples and {len(eval_data)} evaluation examples")
    
    train_examples = build_math_dataset(train_data)
    eval_examples = build_math_dataset(eval_data)
